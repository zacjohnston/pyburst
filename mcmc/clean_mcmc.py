...
source_path = os.path.join(GRIDS_PATH, 'sources', source, 'mcmc')
file_list = os.listdir(source_path)
discard = []
keep = []
for file_ in file_list:
    if ((keep_step not in file_) and (file_ not in ['.keep', '.gitignore'])
            and (f'V{version}' in file_)):
            discard += [file_]
    elif (keep_step in file_) and (f'V{version}' in file_):
        keep += [file_]
